...
try:
    ...
    #agent_executor.run("Who mike douglas?")
     # Capture user input using a Streamlit chat input widget and check if there is input
    if prompt := st.chat_input():
        # Display the user's input as a chat message
        st.chat_message("user").write(prompt)

        # Display the assistant's response within a Streamlit chat message
        with st.chat_message("assistant"):
            # Create a new Streamlit callback handler with a container
            st_callback = StreamlitCallbackHandler(st.container())

            # Run the agent with the user's input and specified callbacks
